...
def execute_asr(input_folder, output_folder, model_size, language):
    ...
    for file_name in tqdm(input_file_names):
        try:
            ...
            text = model.generate(input=file_path, language=language)[0]["text"].split('|>')[-1]
            if language == "ja":
                text = g2p(text)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--language", type=str, default="zh", choices=["zh", "yue", "auto", "ja"], help="Language of the audio files."
    )
    parser.add_argument(
