...
from gpt_sovits_tools.my_utils import load_audio
...
def slice(inp, opt_root, threshold, min_length, min_interval, hop_size, max_sil_kept, _max, alpha, i_part, all_part):
    ...
    if os.path.isfile(inp):
        ...
        for i in old_input:
            ...
            if os.path.isdir(i):
                input_subdir = [os.path.join(i, name) for name in sorted(list(os.listdir(i)))]
                for ii in input_subdir:
                    if os.path.isdir(ii):
                        input_subdir2 = [os.path.join(ii, name) for name in sorted(list(os.listdir(ii)))]
                        for iii in input_subdir2:
                            if Path(iii).suffix in (".flac", ".wav"):
                                input.append(iii)

                    elif Path(ii).suffix in (".flac", ".wav"):
                        input.append(ii)
    ...
    for inp_path in input[int(i_part) :: int(all_part)]:
        ...
        try:
            ...
            audio = load_audio(inp_path, 16000)
            # print(audio.shape)
            for chunk, start, end in slicer.slice(audio):  # start和end是帧数
                ...
                #if 6 > sec > 2:
                out_name = xxhash.xxh64(bytes(chunk)).hexdigest()
                print(out_name)
                wavfile.write(
                    "%s/%s_%010d_%010d.wav" % (opt_root, out_name, start, end),
                    16000,
