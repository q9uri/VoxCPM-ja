...
from .lib.ch.zh_normalization.text_normlization import TextNormalizer
...
esupar_parent = str( Path(__file__).parent / "lib/ja/tokenizer" )
if esupar_parent not in sys.path:
    sys.path.insert(0, esupar_parent)
import esupar

nlp=esupar.load(model="./checkpoints/KoichiYasuoka__chinese-roberta-large-upos")

#jieba_fast.setLogLevel(logging.CRITICAL)
#from .lib.ch import jieba as jieba_fast
#from .lib.ch.jieba import posseg as psg
...
#    from .lib.ch.g2pw import G2PWPinyin, correct_pronunciation
#
...
def g2p(segments):
    ...
    for seg in segments:
        ...
        tokenized = nlp(seg)
        words = tokenized.values[1]
        pos_list = tokenized.values[3]
        seg_cut = []
        for i  in range(len(words)):
            seg_cut.append( (words[i], pos_list[i]) )
        ...
        word_phone = "".join(finals)
        out_text_list.append(word_phone)

    print("pypinyin结果", out_text_list)
    phone = " ".join(out_text_list)
    return phone
