...
from ...ja.tokenizer import esupar
...
def parse_table():
    ...
    data = (_ASSETS_DIR / 'table.csv').read_text(encoding='utf8')
...
def annotate(string):
    ...
    for i  in range(len(words)):
        tokens.append( (words[i], pos_list[i]) )
...
def _get_examples():
    '''For internal use'''
    text = (_ASSETS_DIR / 'rules.txt').read_text(encoding="utf8").splitlines()
    examples = []
    for line in text:
        if line.startswith("->"):
...
def get_rule_id2text():
    '''for verbose=True'''
    rules = (_ASSETS_DIR / 'rules.txt').read_text(encoding='utf8')
